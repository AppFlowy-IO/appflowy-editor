...
void main() async {
  group('home - widget test', () {
    ...
    testWidgets('press the home key to go to the start of selected line', (
      ...
      final selection = Selection.collapsed(
        Position(path: [5], offset: text.length),
      );
      await editor.updateSelection(selection);
      ...
      if (Platform.isWindows || Platform.isLinux) {
        expect(editor.selection, Selection.single(path: [5], startOffset: 0));
      }
      //On Mac OS, the document will scroll to the top but the selection
      //will not be updated.
    ...
    testWidgets('press the shift + home to select till beginning of line', (
      ...
      if (!Platform.isMacOS) {
        ...
        expect(
          editor.selection,
          Selection.single(path: [0], startOffset: text.length, endOffset: 0),
        );
