...
void main() async {
  group('deleteCommand - unit test', () {
    group('deleteCommand - collapsed selection', () {
      ...
      test('delete in collapsed selection when the index > 0', () async {
        ...
        // Welcome |to AppFlowy Editor 🔥!
        final selection = Selection.collapsed(Position(path: [0], offset: 8));
      ...
      test(
        '''Delete the collapsed selection when the index is content.length
          and there is a next node that contains a delta
          and the next node is in the same level with the current node''',
        () async {
          final document = Document.blank().addParagraphs(2, initialText: text);
          final editorState = EditorState(document: document);
      ...
      //
      test(
        '''Delete the collapsed selection's next node when cursor is at end
            and current node contains a delta
            and the next node is the child of the current node''',
        () async {
          ...
          final document = Document.blank().addParagraph(
            initialText: text,
            decorator: (index, node) => node.addParagraph(initialText: text),
          );
          final editorState = EditorState(document: document);
    ...
    group('deleteCommand - not collapsed selection', () {
      ...
      test('Delete in the not collapsed selection that is single', () async {
        ...
        final after = editorState.getNodeAtPath([0])!;
        expect(after.delta!.toPlainText(), text.substring(deleteText.length));
      ...
      // Welcome| to AppFlowy Editor 🔥!
      test(
        'Delete in the not collapsed selection that is not single',
        () async {
          final document = Document.blank().addParagraphs(2, initialText: text);
          final editorState = EditorState(document: document);
      ...
      test(
        ...
        () async {
          final document = Document.blank()
              .addParagraph(initialText: text) // Welcome to AppFlowy Editor 🔥!
              .addParagraph(
                initialText: text,
  ...
  group('deleteCommand - widget test', () {
    ...
    testWidgets('Delete the not collapsed selection', (tester) async {
      TestableEditor editor = tester.editor..addParagraph(initialText: text);
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        final selection = Selection(
          ...
          end: Position(path: [1], offset: welcome.length),
        );
        await editor.updateSelection(selection);
    ...
    // |Welcome to AppFlowy Editor 🔥!
    testWidgets(
      'Delete the non-text node after selecting it, such as divider',
      (tester) async {
        final editor = tester.editor
