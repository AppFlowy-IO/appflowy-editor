...
void main() {
  group('Selection update performance test', () {
    ...
    testWidgets(
      'paragraph with 1000 children - tap middle child and measure selection update time',
      (tester) async {
        // Create 1000 child paragraphs
        final children = <Node>[];
        for (int i = 0; i < 1000; i++) {
        ...
        // Find the middle child (at path [0, 500])
        final middleChild = editor.nodeAtPath([0, 500]);
        expect(middleChild, isNotNull, reason: 'Middle child should exist');

        final finder = find.byKey(middleChild!.key);
        expect(
          finder,
          findsOneWidget,
          reason: 'Middle child should be rendered',
        );
        ...
        // Verify selection was updated correctly
        expect(editorState.selection, isNotNull);
        expect(editorState.selection!.start.path, [0, 500]);
        ...
        debugPrint('=' * 60);
        debugPrint('Total children: 1000');
        debugPrint('Tapped: Middle child (path [0, 500])');
        debugPrint('Selection update time: ${elapsedMs}ms');
        debugPrint('=' * 60 + '\n');
    ...
    testWidgets(
      'paragraph with 1000 children - tap last child and measure selection update time',
      (tester) async {
        // Create 1000 child paragraphs
        final children = <Node>[];
        for (int i = 0; i < 1000; i++) {
        ...
        debugPrint('=' * 60);
        debugPrint('Total children: 1000');
        debugPrint('Tapped: Last child (path [0, 999])');
        debugPrint('Selection update time: ${elapsedMs}ms');
        debugPrint('=' * 60 + '\n');
