...
void main() {
  group('getDeltaAttributesInSelectionStart()', () {
    ...
    test(
      'Returns null if selection collapsed at the end of formatted text',
      () {
        expect(
        ...
        final editorState = EditorState(document: formattedDocument);
        editorState.selection = Selection.collapsed(
          Position(path: [0], offset: headingText.length),
        );
        final attributes = editorState.getDeltaAttributesInSelectionStart();
    ...
    test(
      'Returns first node\'s attributes if selection spans multiple nodes',
      () {
        final editorState = EditorState(document: formattedDocument);
