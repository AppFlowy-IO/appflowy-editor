  ...
  group('text_node_parser.dart', () {
    ...
    test('heading style', () {
      for (var i = 1; i <= 6; i++) {
        ...
        expect(
          const HTMLHeadingNodeParser().transformNodeToHTMLString(
            node,
    ...
    test('bulleted list style', () {
      ...
      expect(
        const HTMLBulletedListNodeParser().transformNodeToHTMLString(
          node,
          encodeParsers: parser,
        ),
    ...
    test('empty line', () {
      final node = paragraphNode(attributes: {'delta': []});
      expect(
        const HTMLTextNodeParser().transformNodeToHTMLString(
          node,
          encodeParsers: parser,
        ),
        '<br>',
    ...
    test('numbered list style', () {
      ...
      expect(
        const HTMLNumberedListNodeParser().transformNodeToHTMLString(
          node,
          encodeParsers: parser,
        ),
    ...
    test('todo list style', () {
      ...
      expect(
        const HTMLTodoListNodeParser().transformNodeToHTMLString(
          checkedNode,
          encodeParsers: parser,
        ),
    ...
    test('quote style', () {
      ...
      expect(
        const HTMLQuoteNodeParser().transformNodeToHTMLString(
          node,
          encodeParsers: parser,
        ),
    ...
    test('divider', () {
      ...
      expect(
        const HTMLDividerNodeParser().transformNodeToHTMLString(
          node,
          encodeParsers: parser,
        ),
        '<hr>',
    ...
    test('fallback', () {
      final node = paragraphNode(
        attributes: {
          'delta': (Delta()..insert(text, attributes: {'bold': true})).toJson(),
        },
      );
      expect(
        const HTMLTextNodeParser().transformNodeToHTMLString(
    ...
    test('strikethrough', () {
      final node = paragraphNode(
        attributes: {
          'delta': (Delta()..insert(text, attributes: {'strikethrough': true}))
              .toJson(),
        },
      );
