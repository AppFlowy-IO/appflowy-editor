...
void main() async {
  group('html_nodes_decoder_test.dart', () {
    setUpAll(() {
      TestWidgetsFlutterBinding.ensureInitialized();
    });

    test('parser document', () async {
      final result = NodeHTMLDecoder().convert(rawHTML);

      expect(Document(root: Node(type: "page", children: result)).toJson(),
          example);
    });
    test('quote document', () async {
      final result = NodeHTMLDecoder().convert(rawHtmlWithQuote);

      expect(result.map((e) => e.toJson()).toList(), quoteExample);
    });
    test('nested parser document', () async {
      final result = NodeHTMLDecoder().convert(nestedhtml);

      expect(Document(root: Node(type: "page", children: result)).toJson(),
          nestedDelta);
    });
    test('empty document', () async {
      final result = NodeHTMLDecoder().convert("");
