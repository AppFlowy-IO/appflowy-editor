...
void main() {
  group('NodeAttributesExtensions::', () {
    ...
    test('heading - text is not String return null', () {
      final Attributes attribute = {'subtype': 'heading', 'heading': 123};
    ...
    test('quote', () {
      final Attributes attribute = {'quote': 'quote text'};
      expect(attribute.quote, true);
    });

    test('number - int', () {
      final Attributes attribute = {'number': 99};
      expect(attribute.number, 99);
    });

    test('number - double', () {
      final Attributes attribute = {'number': 12.34};
      expect(attribute.number, 12.34);
    });

    test('number - return null', () {
      final Attributes attribute = {'code': 12.34};
      expect(attribute.number, null);
    });

    test('code', () {
      final Attributes attribute = {'code': true};
      expect(attribute.code, true);
    });

    test('code - return false', () {
      final Attributes attribute = {'quote': true};
      expect(attribute.code, false);
    });

    test('check', () {
      final Attributes attribute = {'checkbox': true};
